...
var Upload = inherit(/** @lends Upload.prototype */ {
        ...
        fs.stat(this._localPath, function (err) {
            ...
            vow
                .all([
                    ...
                    this._getEntries(this._localPath, 'files')
                ])
                .spread(function (directories, files) {
                    this._localDirectories = directories;
    ...
    _mkdir: function (folderName) {
        return this._handlers.getList(path.dirname(folderName), true)
    ...
    _uploadFile: function (filename) {
        ...
        return this._handlers.getList(remoteDir, true)
            .then(function (remoteFolderEntries) {
                var isExist = false;
                if (remoteFolderEntries) {
                    isExist = remoteFolderEntries.some(function (entry) {
                        var size = entry['d:propstat'][0]['d:prop'][0]['d:getcontentlength'];
                        var isFile = Boolean(size);
                        var entryName = decodeURIComponent(entry['d:href']);
                        return isFile && entryName === path.normalize(this._remotePath + filename);
                    }.bind(this));
                }

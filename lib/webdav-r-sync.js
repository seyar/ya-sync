...
var WebdavRSync = inherit(/** @lends WebdavRSync.prototype */ {
    ...
    __constructor: function () {
         ...
         * @type {String}
         */
        this._localRoot = path.normalize(config.localRoot + '/');
         ...
         * @type {String}
         */
        this._remoteRoot = path.normalize(config.remoteRoot + '/');
    ...
    _uploadFile: function (pth) {
        ...
        return this._handlers.getList(dir)
            .then(function (entries) {
                ...
                if (!isExist) {
                    return this._handlers.upload(pth);
                } else {
                    this._showProgress();
                    return false;
                }
            ...
            .then(function (data) {
                if (data === true) {
                    console.info('file %s uploaded', pth);
                }
                return data;
            }, this);
    },
    ...
    _getEntries: function (root, entryType) {
        ...
        readdirp({root: root, entryType: entryType, fileFilter: '!.*'})
            .on('data', function (entry) {
                entries.push(path.basename(this._localRoot) + '/' + entry.path);
            }.bind(this))
            .on('end', function () {
